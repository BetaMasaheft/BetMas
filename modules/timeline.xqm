...
(:general timeline extracted  from all stated dates Item timeline extracted  from all stated dates in item and related entities. first ids are listed with data in a temporary tree and then functions are used to output the javascript as text:)
declare function tl:TimeLine($node as node(), $model as map(*)) {
let $data :=
let $dateManuscripts :=
for $date in collection($config:data-rootMS)//t:origDate[@when or (@notBefore or @notAfter)]
return
    tl:date($date, 'obj')

let $dateInManuscripts :=
for $date in collection($config:data-rootMS)//t:sourceDesc//t:date[@evidence = 'internal'][@when or @notBefore or @notAfter]
return
    tl:date($date, 'obj')

let $dateWorks :=
for $datew in collection($config:data-rootW)//t:sourceDesc//t:date[@when or @notBefore or @notAfter]
return
    tl:date($datew, 'obj')

let $datePersons :=
for $datep in collection($config:data-rootPr)//t:person[t:birth[@evidence = "internal"][@when or @notBefore or @notAfter] or t:death[@evidence = "internal"][@when or @notBefore or @notAfter] or t:floruit[@evidence = "internal"][@when or @notBefore or @notAfter]]
return
    tl:date($datep, 'prs')
        ...
        <links>
            {
                ...
                return
                    ...
                    tl:link($dateManuscript, 'obj', 'g') }
            {
                for $dateWork in $dateWorks
                return
                    tl:link($dateWork, 'work', 'g')
            }
            {
                for $datePerson in $datePersons
                return
tl:link($datePerson, 'prs', 'g')
            }
        </links>
        <dates>
            {
...
let $datesofRelatedManuscripts := for $ref in distinct-values($model('entity')//@ref[not(matches(., '\w{3}\d+\w+'))])
...
let $dateInManuscripts :=
let $dateinthisms := $model('entity')//t:date[@when or @notBefore or @notAfter]
let $datesinRelatedManuscripts := for $ref in distinct-values($model('entity')//@ref[not(matches(., '\w{3}\d+\w+'))])
...
let $datePersons :=
let $datesOfThisPerson := $model('entity')//t:person[t:birth[@evidence = "internal"][@when or @notBefore or @notAfter] or t:death[@evidence = "internal"][@when or @notBefore or @notAfter] or t:floruit[@evidence = "internal"][@when or @notBefore or @notAfter]]
let $datesofRelatedPersons := for $ref in distinct-values($model('entity')//@ref[starts-with(.,'PRS')])
