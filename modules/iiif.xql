...
for $r in $ranges/range
       let $locusrange :=  iiif:locus($r/t:*/t:locus)
...
($iiif:response200,
        ...
        let $msswithimages :=
for $this in collection($config:data-rootMS)//t:TEI[.//t:msIdentifier/t:idno/@facs]
...
(:        look only in institutions:)
  ...
  "attribution": "Provided by BnF, Vatican Library, EthioSPaRe, and other IIIF providers",
  "manifests":  $manifests
...
($iiif:response200,
      ...
      (:       this is where the images actually are, in the images server:)
       let $thumbid := $imagesbaseurl || '_001.tif/full/80,100/0/default.jpg'
...
map {"@context":= "http://iiif.io/api/presentation/2/context.json",
  ...
  "attribution": "Provided by EthioSPaRe project.",
  "logo": map {
    "@id": $config:appUrl ||"/rest/BetMas/resources/images/logo.png"
    },
"rendering": map {
    "@id": $url,
...
($iiif:response200,
let $item := collection($config:data-root || 'ES')//id($id)
let $iiifroot := $config:appUrl ||"/api/iiif/" || $id 
let $imagesbaseurl := $config:appUrl ||'/iiif/' || string($item//t:msIdentifier/t:idno/@facs)
