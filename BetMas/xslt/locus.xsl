<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="#all" version="2.0">
    <xsl:template match="t:locus">
        ...
        <xsl:choose>
<!--            THEN THE REFERENCE IS IN A  WORK RECORD, AND POINTS TO A MANUSCRIPT-->
            <xsl:when test="@corresp">
                <xsl:variable name="externalMainID" select="@corresp"/>
                <xsl:variable name="doc" select="doc(concat('https://betamasaheft.eu/', $externalMainID, '.xml'))"></xsl:variable>
...
<!--                                            if there is an external reference, then add attributes to call the modal containing the images-->
                                            <xsl:when test="$f  and not($text = 'only')">
                                                <xsl:attribute name="onclick">
                                                    <xsl:value-of select="concat('document.getElementById(&#34;images',concat( $externalMainID, replace($f, ' ', '_'), replace($f//ancestor::t:*[@xml:id][1]/@xml:id, '.', '_')), '&#34;).style.display=&#34;block&#34;')"/>
                                                </xsl:attribute>
                                                </xsl:when>
                    ...
                    <div class="w3-modal" id="{$modalid}">
                        ...
                        <div class="w3-modal-content">
                            <header class="w3-container">
                                <h4>Images from <xsl:value-of select="$doc//t:msIdentifier/t:idno/@facs"/>
                                </h4>
                            ...
                            <div class="w3-container">
                                <xsl:variable select="$doc//t:msIdentifier/t:idno/@facs" name="MainFacs"/>
                                <xsl:variable name="mid">
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:value-of select="$externalMainID"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </xsl:variable>
                           ...
                           defaultZoomLevel:   1,</xsl:text>
                                ...
                                -->
                                    <xsl:variable name="f" select="@facs"/>
                                    <xsl:variable name="idnoFacs" select="$doc//t:msIdentifier/t:idno/@facs"/>
                                    <xsl:choose>
...
<!--            -->
<!--            -->
<!--            ALL CASES OF LOCUS IN MANUSCRIPTS-->
        <xsl:otherwise>
            <xsl:choose>
            <!--            deals with the empty use of locus-->
            <xsl:when test="not(text())">
