...
let $temporal := if($d//t:state[@type eq 'existence'][@from or @to]) then for $existence in $d//t:state[@type eq 'existence'][@from or @to] return let $from := string($existence/@from) let $to := string($existence/@to) return ('dcterms:temporal "' ||$from||'/'||$to||'";
 ') else ()
let $PeriodO := if($d//t:state[@type eq 'existence'][@ref]) then for $periodExistence in  $d//t:state[@type eq 'existence'][@ref]  let $periodid := string($periodExistence/@ref) let $period :=  $config:collection-root/id($periodid)[1] return ('dcterms:temporal <' || string(($period//t:sourceDesc//t:ref/@target)[1])|| '>;
 ') else ()
