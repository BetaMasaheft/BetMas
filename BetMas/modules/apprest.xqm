...
let $path := switch($type)
                case 'catalogue' return 
                      ...
                      let $all := $config:collection-rootMS//t:listBibl[@type eq 'catalogue'][descendant::t:ptr[@target eq $catal]]/ancestor::t:TEI
                      let $string := ('$all'|| $key || $languages || $dR || $allMssFilters)
                      return
                util:eval($string)
                case 'repo' return 
                    let $all := $config:collection-rootMS//t:repository[@ref eq $collection ]/ancestor::t:TEI
                    let $string := ('$all'|| $key || $languages || $dR || $allMssFilters)
...
let $context := switch($type)
                case 'catalogue' return ('$config:collection-rootMS//t:listBibl[@type eq "catalogue"][descendant::t:ptr[@target eq "bm:'||$collection||'"]]/ancestor::t:TEI' || $key || $languages || $dR || $allMssFilters)
                case 'repo' return  ('$config:collection-rootMS//t:repository[@ref eq "'|| $collection||'" ]/ancestor::t:TEI' || $key || $languages || $dR || $allMssFilters)
                default return $path 
...
if($mss = '') then ()  else(
    ...
    let $matchingmss := for $ms in tokenize($mss, ',')  
                                    ...
                                    return 
                                          ...
                                          then (
                                         let $titselectMss:= $config:collection-rootMS//t:title[contains(@ref, $ms)]
