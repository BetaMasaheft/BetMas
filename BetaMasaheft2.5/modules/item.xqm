...
let $repoplace := if ($repo//t:settlement[1]/@ref) then titles:printTitleID($repo//t:settlement[1]/@ref) else if ($repo//t:settlement[1]/text()) then $repo//t:settlement[1]/text() else if ($repo//t:country[1]/@ref) then titles:printTitleID($repo//t:country[1]/@ref) else ()
let $reponame := titles:printTitleID($repoID)
let $key := for $ed in $document//t:titleStmt/t:editor[not(@role = 'generalEditor')]/@key  return app:editorKey(string($ed))
...
<div>
                <a target="_blank" href="/manuscripts/{$repoID}/list" role="button" class="btn btn-success btn-sm">{if($repoplace) then ($repoplace, ', ') else ()}
                   {$reponame}</a>
