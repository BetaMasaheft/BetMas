...
switch($name('type'))
case 'user' return 
( log:add-log-message('/user/' || $name('name'), xmldb:get-current-user(), 'error'),
...
case 'xmlitem' return 
( log:add-log-message(request:get-uri(), xmldb:get-current-user(), 'error'),
<html xmlns="http://www.w3.org/1999/xhtml">
    <head></head>
    <body>
...
<div xmlns="http://www.w3.org/1999/xhtml" class="container alert alert-warning">
    ...
    case 'repo' return ( log:add-log-message(request:get-uri(), xmldb:get-current-user(), 'error'),
<h1>{$name('name')} is not the id af any available repository.</h1>,<p class="lead">Available repositories are listed <a href="/institutions/list">here</a>.</p>
    ...
    case 'catalogue' return ( log:add-log-message(request:get-uri(), xmldb:get-current-user(), 'error'),<h1>{$name('name')} is not the id af any available catalogue.</h1>,<p class="lead">Available catalogues are the following:</p>,
    <ul>
    {
...
return
    ...
    case 'item' return ( log:add-log-message(request:get-uri(), xmldb:get-current-user(), 'error'),<h1>{$name('name')} is not an available item.</h1>, <ul><li>Ids are case sensitive.</li><li>Try browsing or using simple or advanced search.</li></ul>)
