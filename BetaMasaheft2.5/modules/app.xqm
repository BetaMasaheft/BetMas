...
(:let $test := console:log('SPARQL query: ' || $query):)
...
%templates:wrap
    ...
    $node as node(), 
    $model as map(*)) {
        let $test := console:log($model("sparqlResult"))
        return
...
let $nOfP := if(empty($numberOfParts) or $numberOfParts = '') then (console:log('empty')) else '[count(descendant::t:msPart) ge ' || $numberOfParts || ']'
