...
(:general graph of all stated relations:)
declare function rels:ALLEntityRels($node as node(), $model as map(*)){

<div><div id="optionsContainer"></div>
                    <div id="BetMasRelView" class="container"/>
                    {
let $listAllRelations :=

let $place := collection($config:data-rootPl)//t:place
let $msItems := collection($config:data-rootMS)//t:msItem/t:title[@ref]

let $persWithRole := collection($config:data-root)//t:persName[@role][@ref [not(.='PRS00000') and not(.='PRS0000')]]


let $relations := collection($config:data-root)//t:relation[@name][(@active and @passive) or @mutual]
(:collect all ids:)

let $ids :=
<allIDS>
{ for $singlePersWithRole in distinct-values($persWithRole/@ref)
return <id>{string($singlePersWithRole)}</id>
}
{ for $MsinSinglePersWithRole in $persWithRole
return <id>{string(root($MsinSinglePersWithRole)/t:TEI/@xml:id)}</id>
}
{ for $msItem in distinct-values($msItems/@ref)
return <id>{string($msItem)}</id>
}
{ for $mssContainingMsItems in $msItems
return <id>{string($mssContainingMsItems//ancestor::t:TEI/@xml:id)}</id>
}
{ for $country in distinct-values($place/t:country/@ref)
return <id>{$country}</id>
}
{ for $region in distinct-values($place/t:region/@ref)
return <id>{$region}</id>
}

{ for $settlement in distinct-values($place/t:settlment/@ref)
...
return
    ...
    (:print nodes counting each id once:)
    <all>
        <ids>
            {
                for $distinctId in distinct-values($ids//id)
                let $collection :=
                switch (substring($distinctId, 0, 4))
                    case 'LOC'
                        return
            ...
            try {
             ...
             then (titles:getGeoNames($distinctId))
                                                ...
                                                (if (contains($distinctId, '#')) 
                                                then substring-before($distinctId, '#') 
                                                else $distinctId ), '.xml'))/t:TEI))
                                
             
                 let $substring := if (string-length($title) gt 20) then concat(substring($title, 0, 20), '...') else $title
              return $substring
    ...
    return
    <edge>{
                        ('{from:"' || (string($singlePersWithRole/@ref)) || '", to:"' || (string(root($singlePersWithRole)/t:TEI/@xml:id)) || '", label:"' || ('bm:'||string($singlePersWithRole/@role)) ||
                        '", font: {align: ''top''}},')
                    }</edge>
    }
    {for $singlePlace in $place[t:region and t:settlement]
    return
    <edge>{
                        ('{from:"' || (string($singlePlace/t:settlement[1]/@ref)) || '", to:"' || (string($singlePlace/t:region[1]/@ref)) || '", label:"' || 'gn:locatedIn' ||
                        '", font: {align: ''top''}},')
                    }</edge>
    }
     {for $singlePlace in $place[t:country and t:settlement]
    return
    <edge>{
                        ('{from:"' || (string($singlePlace/t:settlement[1]/@ref)) || '", to:"' || (string($singlePlace/t:country[1]/@ref)) || '", label:"' || 'gn:locatedIn' ||
                        '", font: {align: ''top''}},')
                    }</edge>
    }
    {for $singlePlace in $place[t:region and t:country]
    return
    <edge>{
                        ('{from:"' || (string($singlePlace/t:region[1]/@ref)) || '", to:"' || (string($singlePlace/t:country[1]/@ref)) || '", label:"' || 'gn:locatedIn' ||
                        '", font: {align: ''top''}},')
                    }</edge>
    }
    {for $msItemInMss in $msItems
    return
    <edge>{
                        ('{from:"' || (string($msItemInMss/@ref)) 
                        || '", to:"' || (string(root($msItemInMss)/t:TEI/@xml:id)) 
                        || '", label:"' || 'saws:formsPartOf' ||
                        '", font: {align: ''top''}},')
                    }</edge>
    }
...
return
<div><p>there are {count($listAllRelations//node)} nodes and {count($listAllRelations//edge)} edges in this graph</p>
<script type="text/javascript">{'var nodes = ['} {distinct-values($listAllRelations/ids//text())}{'];'}

{'var edges = ['}{distinct-values($listAllRelations/relations//text())}{'];'}
</script>
</div>
}
</div>
