...
declare function item:mainRels($this,$collection){
     ...
     let $isSubjectof := 
            ...
            return 
                $corr  
      let $isAuthorof := 
            for $corr in ($w//t:TEI//t:relation[@passive = $id][@name = 'saws:isAttributedToAuthor'], $w//t:TEI//t:relation[@passive = $id][@name = 'dcterms:creator'])
...
<div class="container-fluid col-md-6" id="InMiniatures">
   <h4 class="modal-title">{count($sameKey)} Miniture{if(count($sameKey) gt 1) then 's' else ()} name{if(count($sameKey) gt 1) then () else 's'} person <span class="MainTitle" data-value="{$id}">{$id}</span> </h4>
