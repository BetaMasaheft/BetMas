...
{apprest:scriptStyle()}
 ...
 let $res := 
 let $hits :=  for $resource in collection($config:data-root)/t:TEI[descendant::t:term[@key = $keyword] or descendant::t:title[@type = $keyword] or descendant::t:person[@type = $keyword] or descendant::t:desc[@type = $keyword] or descendant::t:place[@type = $keyword] or descendant::t:ab[@type = $keyword] or descendant::t:faith[@type = $keyword] or descendant::t:occupation[@type = $keyword]]
                         return $resource
   return 
                      map { 
                      'hits' := $hits,
