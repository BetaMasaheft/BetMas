...
<!--                                    images from our server -->
                                <xsl:when test="contains($idnoFacs, 'betamasaheft')">
                                    ...
                                    <xsl:choose>
                                        <xsl:when test="@from and @to">
                                            ...
                                            <xsl:variable name="tiles">
                                                <xsl:for-each select="0 to (xs:integer($count) -1 )">
                                                    <xsl:text>"</xsl:text>
                                                    <xsl:value-of select="concat($iiif,'_', (xs:integer($f) + current()),'.tif/info.json')"/>
                                                    <xsl:text>"</xsl:text>
                                                    <xsl:if test="not(current()=(xs:integer($count) -1 ))">,</xsl:if>
                                                </xsl:for-each>
                                            </xsl:variable>
                                        ...
                                        <xsl:when test="@from and not(@to)">
                                            <xsl:text>"</xsl:text>
                                            <xsl:value-of select="concat($iiif,'_', @facs,'.tif/info.json')"/>
                                            <xsl:text>"</xsl:text>
                                        </xsl:when>
                                        <xsl:when test="@target">
                                            <xsl:for-each select="tokenize(@facs, ' ')">
                                                <xsl:text>"</xsl:text>
                                                <xsl:value-of select="concat($iiif,'_', .,'.tif/info.json')"/>
                                                <xsl:text>"</xsl:text>
                                                <xsl:if test="not(position()=last())">,</xsl:if>
                                            </xsl:for-each>
                                        </xsl:when>
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="@from and @to">
                                                ...
                                                <xsl:variable name="tiles">
                                                    <xsl:for-each select="0 to (xs:integer($count) -1 )">
                                                        <xsl:text>"</xsl:text>
                                                        <xsl:value-of select="concat($iiif, format-number((xs:integer($f) + current()), '0000'),'.jp2/info.json')"/>
                                                        <xsl:text>"</xsl:text>
                                                        <xsl:if test="not(current()=(xs:integer($count) -1 ))">,</xsl:if>
                                                    </xsl:for-each>
                                                </xsl:variable>
