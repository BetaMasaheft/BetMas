...
let $collection := xs:anyURI('/db/apps/BetMas/data/persons') 
             ...
             if(xmldb:collection-available($collection-uri)) then 
                 <response status="okay">
                    ...
                    (
                       ...
                       sm:chgrp(xs:anyURI(concat($collection-uri,'/',$file-name)), 'BetMas/data/persons')
                    )
                    }
                    </message>
                 </response>
...
let $data-collection := '/db/apps/BetMas/data/persons'
    ...
    try {
        ...
        if(matches(request:get-header('User-Agent'), '^GitHub-Hookshot/')) then
            if(request:get-header('X-GitHub-Event') = 'push') then 
                ...
                let $private-key := 
                environment-variable('GIT_SECRETp')
                let $actual-result :=
                    <actual-result>
