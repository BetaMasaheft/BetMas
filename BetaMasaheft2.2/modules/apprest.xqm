...
declare %templates:default('start', 1) %templates:default("per-page", 20) function apprest:listrest($type, $collection, $parameters as map(*), $params) {
...
<div class="form-group">
<div class="input-group">
<input placeholder="go to..." class="form-control" id="GoTo" list="gotohits" autocomplete="on" data-value="{$collection}"/>
...
return
if(count($matchingmss) = 0) then (<p class="lead">Oh no! Among the catalogued manuscripts no one yet has a link to this work. You can still see the record in case you find there usefull informations.</p>,<a class="btn btn-primary" href="{$target-work}"> Go to {$MAINtit}</a>) else 
(
