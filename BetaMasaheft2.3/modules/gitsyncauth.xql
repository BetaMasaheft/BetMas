...
declare function local:do-update($commits as node()*, $contents-url as xs:string?) {
    ...
    return
        ...
        let $collection := xs:anyURI('/db/apps/BetMas/data/authority-files')
            ...
            try {
                (if (xmldb:collection-available($collection)) then
                    ...
                    <response
                        ...
                        <message>
                            {
                                (local:create-collections($collection), xmldb:store($collection, xmldb:encode-uri($file-name), xs:base64Binary($file-data)))
                            }
                        </message>
                    </response>
                ,
                ...
                let $stored-file := doc($collection || '/' || $file-name)
                return
                        ...
                        <message>
                            {
                                ...
                                sm:chgrp(xs:anyURI(concat($collection, '/', $file-name)), 'BetMas/data/authority-files')
                                )
                            }
                        </message>
                    </response>
...
declare function local:do-delete($commits as node()*, $contents-url as xs:string?) {
    ...
    return
        try {
            <response
                status="okay">
                <message>{xmldb:remove($collection, $file-name)}</message>
            </response>
        } catch * {
            <response
                status="fail">
                    ...
                    <xhtml>
                        <html>
                            ...
                            <body>
                                ...
                                <p><b>Please fix these issues as soon as possible and push the data to the <b>authority-files</b> repo again.</b></p>
...
return
    if (not(empty($post-data))) then
        ...
        let $data-collection := '/db/apps/BetMas/data/authority-files'
