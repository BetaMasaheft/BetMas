...
declare function local:do-update($commits as node()*, $contents-url as xs:string?) {
    ...
    return
        ...
        let $collection := xs:anyURI('/db/apps/BetMas/data/manuscripts')
                    ...
                    <xhtml>
                        <html>
                            ...
                            <body>
                                ...
                                <p><b>Please fix these issues as soon as possible and push the data to the <b>manuscripts</b> repo again.</b></p>
...
return
    if (not(empty($post-data))) then
        ...
        let $data-collection := '/db/apps/BetMas/data/manuscripts'
            ...
            try {
                ...
                if (matches(request:get-header('User-Agent'), '^GitHub-Hookshot/')) then
                    if (request:get-header('X-GitHub-Event') = 'push') then
                        ...
                        let $private-key :=
                        environment-variable('GIT_SECRETms')
                        let $actual-result :=
                        <actual-result>
