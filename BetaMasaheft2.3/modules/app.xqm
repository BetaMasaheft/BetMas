    ...
    return
        if (empty($queryExpr) or $queryExpr = "") then
          (if(empty($parameterslist)) then () else ( let $hits := 
             let $path := concat("collection('",$config:data-root,"')//t:TEI", $IDpart, $wt, $repository, $mss, $texts, $script, $support, $material, $bmaterial, $placeType, $personType, $relationType, $keyword, $languages, $scribes, $donors, $patrons, $owners, $parchmentMakers, $binders, $contents, $mss, $authors, $tabots, $genders, $dateRange, $leaves, $wL, $references, $height, $width, $depth, $marginTop, $marginBot, $marginL, $marginR, $marginIntercolumn)
          ...
          let $hits :=
                 ...
                 let $elements : =
                   ...
                   let $path:=    concat("collection('",$config:data-root,"')//t:TEI[",$allels, "]", $IDpart, $wt, $repository, $mss, $texts, $script, $support, $material, $bmaterial, $placeType, $personType, $relationType, $keyword, $languages, $scribes, $donors, $patrons, $owners, $parchmentMakers, $binders, $contents, $mss, $authors, $tabots, $genders, $dateRange, $leaves, $wL, $references, $height, $width, $depth, $marginTop, $marginBot, $marginL, $marginR, $marginIntercolumn)
                    let $test := console:log($path)
                   for $hit in util:eval($path)
