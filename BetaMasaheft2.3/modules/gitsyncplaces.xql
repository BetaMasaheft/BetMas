...
let $req :=
    <http:request http-version="1.1" href="{xs:anyURI($file-path)}" method="GET">
...
let $collection := xs:anyURI('/db/apps/BetMas/data/places') 
                    ...
                    <xhtml>
                        <html>
                            ...
                            <body>
                                ...
                                <p>This time however your newly {$type} file <mark>{$id}.xml</mark> <b>did not validate</b> to the Beta maṣāḥǝft schema,
                                ...
                                <div style="background-color:#ffad99;"><p><b>{$report//status/text()}</b>.</p>
                                {for $mess in $report//message
                                return 
                                <p>{string($mess/@level)} at line {string($mess/@line)}: {$mess/text()}</p>}</div>
                                <p><b>Please fix these issues as soon as possible and push the data to the <b>places</b> repo again.</b></p>
