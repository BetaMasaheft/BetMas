...
declare %private function viewItem:colophon($node as element(t:colophon)) {
    ...
    <hr
        class="colophon"/>,
    <h3 style="scroll-margin-top: 64px;"
        id="{$node/@xml:id}">{
            if ($node/@type) then
                string($node/@type)
            else
...
return:)
    <div>
        <div
            id="transcription{$frag/descendant-or-self::*[@xml:id][1]/@xml:id}" style="word-break: keep-all;">
            {
