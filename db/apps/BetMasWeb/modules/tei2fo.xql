                ...
                if ($node/@target and $node/text()) then
                    ...
                    if ($node/@ref and $node/text()) then
                        ...
                        if ($node/text() or $node/tei:roleName) then
                            ...
                            if ($node/@ref) then
                                <fo:basic-link
                                    external-destination="{$config:appUrl || '/' || string($node/@ref) || '/main'}">↗{exptit:printTitleID($node/@ref)}</fo:basic-link>
...
declare function fo:tei2fo($nodes as node()*) {
    ...
    case element(tei:witness)
        return
            ...
            <fo:block margin-bottom="2mm">{if($node/@xml:id) then <fo:inline font-weight="bold">{(string($node/@xml:id) || ': ')}</fo:inline> else ()} {if($node/@type eq  'external') then <fo:basic-link
                        external-destination="{string($node/@facs)}" font-weight="bold">{string($node/@corresp)}</fo:basic-link> else exptit:printTitleID(string($node/@corresp))}</fo:block>
            ...
            <fo:block margin-top="5mm">
               ...
               let $corresps := for $cor in $corr
                ...
                let $correspsEl :=  for $c in $corresps 
                               ...
                               (if($ref/@xml:lang) then concat(' [', $file//tei:language[@ident eq  $ref/@xml:lang], ']') else ()))
                return (' (about: ' || 
    ...
    case element(tei:lb)
        return
('&#x000A;', <fo:inline margin-left="15mm">{if(number($node/@n) and $node/@n mod 5 = 0 and not($node/@n eq  0)) then string($node/@n) else(' ')}</fo:inline>)
                    ...
                    <fo:list-item-body
                        ...
                        <fo:block>{
                                if ($addition/tei:desc/@type) then
                                    <fo:inline
                                        font-weight="bold">{exptit:printTitleID(string($addition/tei:desc/@type))}</fo:inline>
                                else
                                    ()
                    ...
                    <fo:list-item-body
                        ...
                        <fo:block>{
                                if ($deco/tei:desc/@type) then
                                    <fo:inline
                                        font-weight="bold">{exptit:printTitleID(string($deco/tei:desc/@type))}</fo:inline>
                                else
                                    ()
                        ...
                        <fo:table-row
                            ...
                            <fo:table-cell><fo:block>{
                                        ...
                                        return
                                            ...
                                            <fo:inline>{if($dim/following-sibling::tei:note[@corresp eq  $dimID]) then (fo:tei2fo($dim/following-sibling::tei:note[@corresp eq  $dimID])) else ()}</fo:inline>
                                            )
                               ...
                               ,
                                ...
                                if ($file//tei:div[@type eq 'bibliography']/tei:listBibl) then
                                    <fo:block>
                                        ...
                                        <fo:block>
                                            {fo:tei2fo($file//tei:div[@type eq 'bibliography']/tei:listBibl)}</fo:block>
                                    </fo:block>
                                else
                                    (),
                                if ($file//tei:div[@type eq 'edition']) then
                                    <fo:block>
                                        ...
                                        <fo:block>
                                            {fo:tei2fo($file//tei:div[@type eq 'edition']/node())}</fo:block>
                                    </fo:block>
                                else
                                    ())
                               ...
                               ,
                                ...
                                if ($file//tei:div[@type eq 'bibliography']/tei:listBibl) then
                                    <fo:block>
                                        ...
                                        <fo:block>
                                            {fo:tei2fo($file//tei:div[@type eq 'bibliography']/tei:listBibl/node())}</fo:block>
                                    </fo:block>
                                else
                                    ())
