...
declare %private function local:split-path ($path) {
	...
	return map {"basename": $filename, "directory": substring-before($path, "/" || $filename)}
};
...
declare function local:should-handle-file ($path as xs:string) {
	$path => ends-with(".xml") and
		not($path => ends-with("expath-pkg.xml")) and
		not($path => ends-with("repo.xml")) and
		not($path => ends-with("taxonomy.xml"))
};
...
:)
declare function tuttle-expand:expand-data ($collection-config as map(*), $changes as map(*)) {
	...
	let $unexpanded-root := "/db/apps/" || $collection-config?collection
	return ( (: Remove old stuff :)
			for $removed-file in $changes?del?*
			where $removed-file?success and local:should-handle-file($removed-file?path)
			let $path-in-expanded := $expanded-root || "/" || $removed-file?path
			let $descriptor-in-expanded := local:split-path($path-in-expanded)
			let $_ := util:log("info", "Removing removed file " || $path-in-expanded)
			return if (not(doc-available($path-in-expanded))) then
					util:log("warning", "File " || $removed-file?path || " does not exist in expanded data")
				else
			...
			where $inserted-file?success and local:should-handle-file($inserted-file?path)
			...
			return if ($expanded) then (
					util:log("info", "Expanding " || $filepath || " into " || $filepath-in-expanded), try {
					...
					util:log("warn", "Expanding " || $filepath || " into " || $filepath-in-expanded || "failed. Skipping.")
				)
		)
};
