...
declare variable $local:ms := collection($config:data-rootMS) ;
...
<div><div class="msscomparison col-md-12">
{
        for $ms in $mss
        return
    try {    let $manuscript := $local:ms//id($ms)
...
<table class="table table-responsive">
        ...
        <tbody>{
        let $mssfiles := for $ms in $mss return $local:ms//id($ms)
...
<div class="whichTextualUnitsHaveTitles col-md-12">{
        ...
        let $manuscripts := for $ms in $mss return $local:ms//id($ms)
       ...
       {for $ms in $mss 
       let $manuscriptFile := $local:ms//id($ms)
       return 
       <td>
       {
