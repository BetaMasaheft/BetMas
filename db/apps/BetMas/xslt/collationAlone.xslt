<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:template match="t:collation">
        ...
        <h3>
            <span class="w3-tooltip">Quire Structure <span class="w3-text w3-tag w3-gray"
                    >Collation</span></span>
            <xsl:if test="./ancestor::t:msPart">
        ...
        <xsl:if test="t:list">
            <div class="w3-container allCollation">
                ...
                <button type="button" class="w3-button w3-red"
                    onclick=" openAccordion('collation{$mspartID}')">Quire Table</button>
                    ...
                    <table class="w3-table">
                        ...
                        <xsl:for-each select=".//t:item">
                            ...
                            <tr>
                                ...
                                <td>
                                    <xsl:apply-templates
                                        select="child::node() except (t:locus | t:dim)"/>
                                </td>
                            </tr>
                        </xsl:for-each>
                    </table>
                ...
                <xsl:choose>
                    ...
                    <xsl:when
                        test="$dimensionandstubs//item[child::dimensions[not(@xml:lang)][. mod 2 = 0]][child::stubs[. mod 2 != 0]]">
                        <div class="w3-panel w3-black">
                            ...
                            <ul>
                                <xsl:for-each
                                    select="$dimensionandstubs//item[child::dimensions[not(@xml:lang)][. mod 2 = 0]][child::stubs[. mod 2 != 0]]">
                                    <li>
                                        <xsl:value-of
                                            select="concat('Quire with id:', id, ' and n ', n, ' is made of an even number of leaves (', dimensions, '), but the number of stubs is odd.')"
                                        />
                                    </li>
                                </xsl:for-each>
                            </ul>
                    ...
                    <xsl:when
                        test="not(.//t:list/t:item[not(matches(text()[last()], '\d+'))][./t:dim[@unit = 'leaf'][. mod 2 != 0]])">
                        ...
                        <xsl:if test="$step3">
                            <xsl:for-each select="$step3">
                                <xsl:call-template name="visColl">
                                    <xsl:with-param name="Finalvisualization" tunnel="yes"
                                        select="."/>
                                </xsl:call-template>
                            </xsl:for-each>
                        </xsl:if>
                        ...
                        <!--        formula-->
                        <button type="button" class="w3-button w3-red"
                            onclick=" openAccordion('collationFormula{$mspartID}')">Collation
                            Formula </button>
                        <div id="collationFormula{$mspartID}" class="w3-container w3-hide">
                            <p>Ethio-SPaRe formula : <xsl:for-each select=".//t:item">
                                    ...
                                    <xsl:choose>
                                        <xsl:when test="contains(string-join(text()), 'stub')">
                                            ...
                                            <xsl:variable name="countstubs"
                                                select="string-length($stubs)"/>
                                            <xsl:value-of
                                                select="number(t:dim[1]) - number($countstubs)"
                                                />+<xsl:value-of select="$countstubs"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                    ...
                                    <xsl:if test="contains(string-join(text()), 'stub')">
                                        ...
                                        <xsl:variable name="stubs">
                                            <xsl:analyze-string select="$text"
                                                regex="((\d+)(,?)(\s?)(no\s)?((stub|added|replaced|missing)(\s+)?)(after|before)?(\s?)(\d+)?)">
                                                <xsl:matching-substring><single>s.l. <xsl:value-of
                                                  select="regex-group(1)"
                                                  /></single></xsl:matching-substring>
                                            </xsl:analyze-string>
                                        </xsl:variable>
                            ...
                            <p>Formula 1: <xsl:for-each select="$visColl//t:quire">
                                    ...
                                    <xsl:value-of select="$quire-no"/> (<xsl:value-of
                                        select="$no-leaves"/>
                            ...
                            <p>Formula 2: <xsl:for-each select="$visColl//t:quire">
                                    ...
                                    <xsl:for-each select="child::t:leaf[@mode = 'missing']">
                                        <xsl:choose>
                                            <xsl:when test="preceding-sibling::t:leaf">, leaf
                                                missing after fol. <xsl:value-of
                                                  select="preceding-sibling::t:leaf[1]/@folio_number"
                                                />
                                            </xsl:when>
                                    ...
                                    <xsl:for-each select="child::t:leaf[@mode = 'replaced']">
                                        <xsl:choose>
                                            <xsl:when test="preceding-sibling::t:leaf">, leaf
                                                replaced after fol. <xsl:value-of
                                                  select="preceding-sibling::t:leaf[1]/@folio_number"
                                                />
                                            </xsl:when>
                    ...
                    <xsl:otherwise>
                        <div class="w3-panel w3-black">
                            ...
                            <ul>
                                <xsl:for-each
                                    select="//t:list/t:item[not(matches(text()[last()], '\d+'))][./t:dim[@unit = 'leaf'][. mod 2 != 0]]">
                                    <li>
                                        <xsl:value-of
                                            select="concat('Quire with id:', @xml:id, ' and n ', @n, ' is made of ', t:dim[@unit = 'leaf']/text(), ' leaves, but there is no futher information about the reason this number is odd.')"
                                        />
                                    </li>
                                </xsl:for-each>
                            </ul>
