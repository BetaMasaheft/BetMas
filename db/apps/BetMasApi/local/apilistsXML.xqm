...
(:~
 ...
 :)
declare function apiLx:collection($request as map(*)) {
	...
	return (: if ($perpage gt 100)
            then (<info>Try a lower value for the parameter perpage. Maximum is 100.</info>)
    else :) let $test := console:log("got here")
						...
						switch ($resource/@type)
							case "mss" return
								(
									...
									attribute scribe {
										for $r in
											$resource//t:persName[@role eq "scribe"]/@ref[not(. eq "PRS00000") and not(. eq "PRS0000")]
										return string($r) || " "
									},
									attribute donor {
